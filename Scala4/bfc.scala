...
def jumpRight(prog: String, pc: Int, level: Int) : Int = {
  if(prog.length <= pc) pc
      ...
      case (']', 0) => pc + 1
      case ('[', _) => jumpRight(prog, pc + 1, level + 1)
      case (']', _) => jumpRight(prog, pc + 1, level - 1)
      case (_, _) => jumpRight(prog, pc + 1, level)
  }
}
...
def jumpLeft(prog: String, pc: Int, level: Int) : Int = {
  if(pc < 0) pc
      ...
      case ('[', 0) => pc + 1
      case ('[', _) => jumpLeft(prog, pc - 1, level - 1)
      case (']', _) => jumpLeft(prog, pc - 1, level + 1)
      case (_, _) => jumpLeft(prog, pc - 1, level)
...
def jump(prog: String, pc: Int, mp: Map[Int, Int]) : Map[Int, Int] = {
  if(prog.length > pc && pc >= 0) { 
    (prog(pc)) match {
      case '[' => jump(prog, pc + 1, mp + (pc -> jumpRight(prog, pc + 1, 0)))
      case ']' => jump(prog, pc + 1, mp + (pc -> jumpLeft(prog, pc - 1, 0)))
      case _ => jump(prog, pc + 1, mp)
    } 
  } else mp
}
...
def compute2(pg: String, tb: Map[Int, Int], pc: Int, mp: Int, mem: Mem) : Mem = {
  if(pg.length > pc && pc >= 0) (pg(pc)) match {
