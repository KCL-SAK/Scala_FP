...
def simp(r: Rexp) : Rexp = r match{
  case SEQ(r1, r2) => SEQ(simp(r1), simp(r2)) match {
    ...
    case SEQ(r3, ONE) => r3
    case SEQ(ONE, r4) => r4
    case SEQ(r3, r4) => SEQ(r3, r4)
  }

  case ALT(r1, r2) => ALT(simp(r1), simp(r2)) match {
    case ALT(r3, ZERO) => r3
    case ALT(ZERO, r4) => r4
    case ALT(r3, r4) if (r3 == r4) => r3
    case ALT(r3, r4) => ALT(r3, r4)
  }
...
def ders (s: List[Char], r: Rexp) : Rexp = s match {
  case (Nil) => r
  case (c::sc) => ders(sc, simp(der(c, r)))
}
...
println("test 1: " + matcher(("a" ~ "b") ~ "c", "abc"))  // => true
println("test 2: " + matcher(("a" ~ "b") ~ "c", "ab"))  // => false
val EVIL = SEQ(STAR(STAR(CHAR('a'))), CHAR('b'))
println("test 3: " + size(der('a', der('a', EVIL))))
println("test 4: " + size(der('a', der('a', der('a', EVIL)))))
println("test 5: " + size(simp(der('a', der('a', EVIL)))))
println("test 6: " + size(simp(der('a', der('a', der('a', EVIL))))))
