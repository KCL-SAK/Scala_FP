...
def get_csv_url(url: String) : List[String] = {
    Source.fromURL(url).getLines.drop(1).toList
    //Try(Some(Source.fromURL(url).getLines.drop(1).toList)).getOrElse("")
}
...
def process_ratings(lines: List[String]) : List[(String, String)] = {
    ...
    val temp = lines.filter(_.split(",").last.toInt >= 4)
...
// println(""+good_ratings.length)
// println(""+movie_names.length)
...
def groupById(ratings: List[(String, String)],  m: Map[String, List[String]]) : Map[String, List[String]] = { 
    ratings match {
        case Nil => m
        case x::xs => {
          val map = m + (x._1 -> (x._2::m.getOrElse(x._1,List())))
          groupById(xs, map)
        }
    }
}
...
def favourites(m: Map[String, List[String]], mov: String) : List[List[String]] = {
  val list = m.toList.filter(x => x._2.contains(mov))
  list.map(x => x._2).map(x => x.filter(_!=mov))
}
...
def suggestions(recs: Map[String, List[String]],mov_name: String) : List[String] = {
  val results = favourites(recs, mov_name).flatten
  occurrences(results).toList.sortBy(_._2).map(_._1)
}
