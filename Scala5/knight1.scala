...
def is_legal(dim: Int, path: Path, x: Pos) : Boolean = {
  if(x._1 < 0 || x._2 < 0 || x._1 >= dim || x._2 >= dim || path.contains(x)) false else true
}
...
def legal_moves(dim: Int, path: Path, x: Pos) : List[Pos] = {
  ...
  moves.filter(x => is_legal(dim, path, x))
}
...
def count_tours(dim: Int, path: Path) : Int = {
  ...
  val x = legal_moves(dim, path, path.head)
  (for(moveHead<-x) yield {
    count_tours(dim, moveHead::path)
  }).sum
  }
}
//x.map(x => count_tours(dim, x::moveHead)).sum
...
def first_tour(dim: Int, path: Path) : Option[Path] = {
  if(path.length == (dim)*(dim)) Some(path) 
    ...
    first(legal, (pos: Pos)=>first_tour(dim, pos::path))
  }
}
