...
def get_january_data(symbol: String, year: Int) : List[String] = {
   val file = Source.fromFile(symbol ++ ".csv").getLines.toList
   file.filter(_.startsWith(year.toString))
}
//println("hello" + get_january_data("GOOG", 2010))
...
def get_first_price(symbol: String, year: Int) : Option[Double] = {
   //Try((Some(Source.fromFile(symbol).getLines.toList).filter(_.startsWith(year + "")).toList).last).getOrElse(None)
...
def get_prices(portfolio: List[String], years: Range) : List[List[Option[Double]]] = {
   for(year <- years.toList) yield for(company <- portfolio) yield get_first_price(company, year)
}
...
def get_deltas(data: List[List[Option[Double]]]) :  List[List[Option[Double]]] = {
    for (x <- (0 until data.length-1).toList) yield  {
        for (y <- (0 until data.head.length).toList) yield {
            get_delta(data(x)(y),data(x+1)(y))
        }
    }
}
...
def yearly_yield(data: List[List[Option[Double]]], balance: Long, index: Int) : Long = {
   val changes = data(index).flatten
   val noOfCompanies = changes.length
   val investAmount = balance/noOfCompanies
  
   if(data(index).flatten.length > 0) {
      changes.map(x => x*investAmount).sum.toLong + balance
   } else balance
}
